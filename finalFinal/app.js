...
function updateAndRender() {
    ...
    const sortedGeometry = sortDistance(sphereGeometryList, camera.getPosition());
    // todo #6
    // uncomment when directed by guide
    for (var i = 0; i < sphereGeometryList.length; ++i) {
        const object = sortedGeometry[i];
        object.render(camera, projectionMatrix, textureShaderProgram);
    }
...
function sortDistance(objects, cameraPos){
    return objects.sort((a, b) => {
        const positionA = a.getPosition();
        const positionB = b.getPosition();

        let distanceA = positionA.subtract(cameraPos).lengthSqr();
        let distanceB = positionB.subtract(cameraPos).lengthSqr();
